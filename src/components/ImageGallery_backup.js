...
const ImageGallery = ({ exhibitionId }) => {
  ...
  const handleNext = () => {
    ...
    const newNextIndex = (currentIndex + 1) % totalItems; // Calculate the next index
  ...
  const handleBack = () => {
    if (isTransitioning) return; // Prevent multiple transitions
    setIsTransitioning(true);
    const newNextIndex = (currentIndex - 1 + totalItems) % totalItems; // Calculate the previous index
    setNextIndex(newNextIndex);
    setTimeout(() => {
      setCurrentIndex(newNextIndex);
      setIsTransitioning(false);
